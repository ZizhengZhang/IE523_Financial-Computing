...
// Created by Zizheng Zhang
// Some hints are written by Prof. R.S. Sreenivas
...
void print_the_solution(int n, double solution[])  // Print the final solution
{
	cout << endl << "Final Solution #" << n << endl;
	for (int i = 1; i <= 9; i++)
	{
		for (int j = 1; j <= 9; j++)
		{
...
{
	int ret;
	int n = 0;  // The number of all the solutions
	...
	while (ret == 0)
	{
		...
		// print the solution
		print_the_solution(n, solution);
		
		// Add this solution to the constraints
		double row[730];
		row[0] = 0;
		for (int i = 1; i < 730; i++)
			row[i] = solution[i-1];
